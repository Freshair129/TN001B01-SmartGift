(async()=>{
  ...
  if(videos.length!==2 || videos.some(v=>v.readyState<2 || v.duration!==4 || !v.currentSrc.endsWith('?v=5'))) throw new Error('Wrong or unready deployed media');
  ...
  const bounds={left:x+629.7103*scale,top:y+55.5619*scale,right:x+1290.2897*scale,bottom:y+1014.7518*scale};
  ...
  const reverse=[]; for (const t of [3.966,2,.6,0]) { for(const v of videos){v.currentTime=t;} await sleep(200); reverse.push(videos.map(v=>v.currentTime)); if(videos.some(v=>Math.abs(v.currentTime-t)>.04))throw new Error('Reverse seek failed'); }
  return {status:'PASS',reverse,viewport:[innerWidth,innerHeight],left,right,center,closedDifferentChannels:differences,
