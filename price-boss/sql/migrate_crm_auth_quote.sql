-- ============================================================================
-- Price Boss CRM + Auth + Quotation (canonical)
-- Database: smartgift  ·  Roles: administrator | manager | sale
-- Safe to re-run (IF NOT EXISTS / ON DUPLICATE KEY / INSERT IGNORE)
--
-- Apply:
...
CREATE TABLE IF NOT EXISTS permissions (
  ...
  module      VARCHAR(64)  NOT NULL COMMENT 'customer|quote|product|employee|admin|sourcing',
...
CREATE TABLE IF NOT EXISTS employees (
  ...
  password_hash VARCHAR(255) NULL COMMENT 'เก็บ bcrypt/argon2 hash เท่านั้น — ห้าม plain text',
...
CREATE TABLE IF NOT EXISTS customers (
  ...
  owner_emp_id    BIGINT UNSIGNED NULL COMMENT 'พนักงานขายเจ้าของลูกค้า (sale scope)',
...
CREATE TABLE IF NOT EXISTS quotations (
  ...
  owner_emp_id    BIGINT UNSIGNED NOT NULL COMMENT 'พนักงานขายที่ออกใบ (sale scope)',
  status          ENUM(
                    'draft',
                    'submitted',
                    'approved',
  ...
  valid_until     DATE NULL COMMENT 'วันหมดอายุยืนราคา (ค่าเริ่มต้น +30 วัน)',
  ...
  customer_note   TEXT NULL COMMENT 'ข้อความถึงลูกค้า / เงื่อนไขพิมพ์',
...
CREATE TABLE IF NOT EXISTS quotation_items (
  ...
  qty             DECIMAL(12,2) NOT NULL DEFAULT 1 COMMENT 'จำนวนที่เสนอหลัก',
  qty_tier        INT NULL COMMENT 'ขั้นราคาหลักที่เลือก 10/20/50/100/500',
  unit            VARCHAR(32) NOT NULL DEFAULT 'ชิ้น',
  unit_price      DECIMAL(14,2) NOT NULL DEFAULT 0 COMMENT 'ราคาต่อหน่วยของขั้นที่เลือก',
  discount_amt    DECIMAL(14,2) NOT NULL DEFAULT 0,
  line_total      DECIMAL(14,2) NOT NULL DEFAULT 0,
  branding_note   VARCHAR(512) NULL COMMENT 'โลโก้ / สี / packing',
  carton_note     VARCHAR(255) NULL COMMENT 'เช่น 20 ชุด/กล่อง · 0.12 CBM',
  profile_code    VARCHAR(32) NULL COMMENT 'standard|corporate จากเครื่องคำนวณ',
...
-- บันไดจำนวน/ราคาต่อบรรทัด (ตรง UI ใบเสนอราคา SmartGift)
CREATE TABLE IF NOT EXISTS quotation_line_breaks (
  id                 BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  quotation_item_id  BIGINT UNSIGNED NOT NULL,
  qty                INT NOT NULL COMMENT 'จำนวนชุด เช่น 100/300/500/1000',
  unit_price         DECIMAL(14,2) NOT NULL COMMENT 'ราคาต่อชุด (THB)',
  sort_order         INT NOT NULL DEFAULT 0,
  created_at         DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uk_qlb_item_qty (quotation_item_id, qty),
  KEY idx_qlb_item (quotation_item_id),
  CONSTRAINT fk_qlb_item FOREIGN KEY (quotation_item_id) REFERENCES quotation_items (id)
    ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- ═══════════════════════════════════════════════════════════════════════
-- 5) Seed roles / permissions / demo employees
-- ═══════════════════════════════════════════════════════════════════════
...
INSERT INTO roles (code, name_th, name_en, description) VALUES
  ...
  ('manager',       'ผู้จัดการ',   'Manager',       'อนุมัติใบเสนอราคา ดูทุกใบ/ทุกลูกค้า จัดการสินค้า — ไม่จัดการ user/role'),
  ('sale',          'พนักงานขาย', 'Sale',          'สร้าง/แก้ไขลูกค้าและใบเสนอราคาของตนเองเท่านั้น')
...
INSERT INTO permissions (code, module, name_th, description) VALUES
  ('customer.read',     'customer', 'ดูลูกค้า (ของตน)',     'sale: เฉพาะ owner_emp_id = ตน · manager/admin: ใช้ร่วมกับ read_all'),
  ('customer.read_all', 'customer', 'ดูลูกค้าทั้งหมด',     'ข้าม owner scope'),
  ('customer.write',    'customer', 'แก้ลูกค้า (ของตน)',    'สร้าง/แก้ไขลูกค้าใน scope'),
  ('customer.write_all','customer', 'แก้ลูกค้าทั้งหมด',    'แก้ไขลูกค้าทุก owner'),
  ('customer.delete',   'customer', 'ลบ/ปิดลูกค้า',        'ลบหรือตั้ง inactive/blacklist'),
  ('quote.read',        'quote',    'ดูใบเสนอราคา (ของตน)', 'sale: เฉพาะ owner_emp_id = ตน'),
  ('quote.read_all',    'quote',    'ดูใบเสนอราคาทุกคน',   'ข้าม owner scope'),
  ('quote.write',       'quote',    'แก้ใบเสนอราคา (ของตน)','สร้าง/แก้ draft ใน scope'),
  ('quote.submit',      'quote',    'ส่งอนุมัติ',           'draft → submitted'),
  ('quote.approve',     'quote',    'อนุมัติ/ตีกลับ',       'submitted → approved|rejected'),
  ('quote.send',        'quote',    'ส่งให้ลูกค้า',         'approved → sent'),
  ('quote.cancel',      'quote',    'ยกเลิกใบเสนอราคา',     '→ cancelled (manager/admin)'),
  ('product.read',      'product',  'ดูสินค้า/ราคา',        'catalog + smartgift price'),
  ('product.write',     'product',  'แก้สินค้า',            'แท็ก / master สินค้า'),
  ('sourcing.write',    'sourcing', 'แก้ China sourcing',   'china_sourcing / suppliers'),
  ('employee.read',     'employee', 'ดูพนักงาน',            'รายชื่อพนักงาน'),
  ('employee.write',    'employee', 'จัดการพนักงาน',        'สร้าง/แก้พนักงาน (= employee.manage)'),
  ('employee.manage',   'employee', 'จัดการพนักงาน (alias)', 'alias ของ employee.write'),
  ('role.manage',       'admin',    'จัดการบทบาท/สิทธิ์',   'roles / permissions'),
  ('admin.access',      'admin',    'เข้าถึงระบบสิทธิ์',    'หน้า admin RBAC')
ON DUPLICATE KEY UPDATE
...
-- manager: ทุกข้อมูลขาย + อนุมัติ/ยกเลิก — ไม่จัดการ user/role
INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN (
  'customer.read','customer.read_all','customer.write','customer.write_all',
  'quote.read','quote.read_all','quote.write','quote.submit','quote.approve','quote.send','quote.cancel',
  'product.read','product.write','sourcing.write',
  'employee.read'
) WHERE r.code = 'manager';

-- sale: ของตนเองเท่านั้น
INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN (
...
-- Demo users — ตั้ง password_hash จริงก่อนเปิด login production
-- Placeholder: ว่าง = ยังไม่ใช้ password auth (dev login อาจใช้ username อย่างเดียว)
...
-- Upgrade columns on existing quotation_items (compatible with MySQL < 8.0.29)
-- ADD COLUMN IF NOT EXISTS is not supported on older servers — use information_schema.
SET @pb_db := DATABASE();

SET @pb_sql := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE quotation_items ADD COLUMN carton_note VARCHAR(255) NULL COMMENT ''carton / CBM note'' AFTER branding_note',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = @pb_db AND TABLE_NAME = 'quotation_items' AND COLUMN_NAME = 'carton_note'
);
PREPARE pb_stmt FROM @pb_sql; EXECUTE pb_stmt; DEALLOCATE PREPARE pb_stmt;

SET @pb_sql := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE quotation_items ADD COLUMN profile_code VARCHAR(32) NULL COMMENT ''standard|corporate'' AFTER carton_note',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = @pb_db AND TABLE_NAME = 'quotation_items' AND COLUMN_NAME = 'profile_code'
);
PREPARE pb_stmt FROM @pb_sql; EXECUTE pb_stmt; DEALLOCATE PREPARE pb_stmt;
...
SELECT
  ...
  c.owner_emp_id AS customer_owner_emp_id,
  ...
  (SELECT COUNT(*) FROM quotation_line_breaks b
     JOIN quotation_items qi2 ON qi2.id = b.quotation_item_id
    WHERE qi2.quotation_id = q.id) AS break_count
FROM quotations q
